...
import { TbExternalLink, TbMail, TbPhone } from "react-icons/tb";
      ...
      <div className="grid grid-cols-12 gap-2 h-full w-screen bg-gray-100">
        <div className="flex flex-col lg:col-span-4 sm:col-span-6 col-span-12 px-4">
          <div className="relative py-4 flex flex-col items-center justify-start mt-32 mb-2 h-full bg-white rounded-lg space-y-3 md:space-y-6 px-6 border">
            ...
            <div>
              <span className="my-1 text-gray-800 text-sm font-medium me-2 px-2.5 py-0.5 rounded flex bg-gray-50 border">
                <span className="px-1 text-white">
                  <DiHtml5Connectivity className="w-5 h-5 text-gray-700" />
                </span>
            ...
            <div className="flex flex-wrap items-center mt-1">
              {Object.keys(vinfo.socialProfils).map((so: any, i) => (
                <span className="mr-3 mt-3" key={i}>
                  {vinfo.socialProfils[so].uri &&
                    vinfo.socialProfils[so].type == "instagram" && (
                      <Link
                        href={vinfo.socialProfils[so].uri}
                        target="_blank"
                  ...
                  {vinfo.socialProfils[so].uri &&
                    vinfo.socialProfils[so].type == "facebook" && (
                      <Link
                        href={vinfo.socialProfils[so].uri}
                        target="_blank"
                        className="border h-12 w-12 rounded-md flex items-center justify-center"
                  ...
                  {vinfo.socialProfils[so].uri &&
                    vinfo.socialProfils[so].type == "linkedin" && (
                      <Link
                        href={vinfo.socialProfils[so].uri}
                        target="_blank"
                        className="border h-12 w-12 rounded-md flex items-center justify-center"
                  ...
                  {vinfo.socialProfils[so].uri &&
                    vinfo.socialProfils[so].type == "youtube" && (
                      <Link
                        href={vinfo.socialProfils[so].uri}
                        target="_blank"
                        className="border h-12 w-12 rounded-md flex items-center justify-center"
                  ...
                  {vinfo.socialProfils[so].uri &&
                    vinfo.socialProfils[so].type == "tiktok" && (
                      <Link
                        href={vinfo.socialProfils[so].uri}
                        target="_blank"
                        className="border h-12 w-12 rounded-md flex items-center justify-center"
                  ...
                  {vinfo.socialProfils[so].uri &&
                    vinfo.socialProfils[so].type == "twitter" && (
                      <Link
                        href={vinfo.socialProfils[so].uri}
                        target="_blank"
                        className="border h-12 w-12 rounded-md flex items-center justify-center"
            ...
            <div className="flex space-x-3 md:pb-2 md:relative md:w-full md:shadow-none shadow py-4 fixed md:bg-white bg-noir-bold justify-center items-center bottom-0 left-0 w-screen">
              <Button outlined={true} onClick={() => {}}></Button>
            </div>
          </div>
        </div>
