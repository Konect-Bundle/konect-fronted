...
import {Button, Checkbox, type CustomFlowbiteTheme, Dropdown, Label, TextInput} from "flowbite-react";
...
export default function CountryCode({callback}: KuserBlockProps) {
    ...
    return <span>
        <Dropdown label={nodeMaker(current)} inline theme={customDropdown}>
            {availablesCountries.map(({name, countryCode, icon}, index) => <Dropdown.Item key={index} onClick={() => {
                setCurrent(index)
            }}>
                {nodeMaker(index)}
            </Dropdown.Item>)}
        ...
        <TextInput type="text" disabled className="hidden invisible opacity-0" id="countryCode"
