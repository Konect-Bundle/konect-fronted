...
export default function PasswordResetPage(props: IPasswordResetPageProps) {
    ...
    const handleSubmitForm = (values: { oldPassword: string, newPassword: string, passwordRe: string }) => {
        setIsLoading(true);

        var token = getCookie(AUTH_TOKEN_NAME);

        if (token) {
            UserService.updatePassword(values.newPassword, values.oldPassword, token).then((rs) => {
                setIsLoading(false);
    ...
    return (
        user && <section className="grid grid-cols-8 gap-4 lg:pt-12 md:pt-4 pt-4">
            ...
            <div className="lg:col-span-6 col-span-8">
                <LoadingLayout isLoading={isLoading}>
                    <Formik initialValues={{ oldPassword: "", newPassword: "", passwordRe: "" }} onSubmit={handleSubmitForm}>
                        {(formProps) => (
                            <Form className="flex flex-col items-end">
                                <div className="bg-white rounded-lg mb-6 w-full p-8">
                                    ...
                                    <div className="mt-3">
                                        <InputWithLabel
                                            labelFor={"passwordRe"}
                                            labelTitle={__("password_confirm")}
                                        >
                                            <InputField
                                                labelFor="passwordRe"
                                                name="passwordRe"
